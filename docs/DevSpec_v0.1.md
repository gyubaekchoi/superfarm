...
## 12. 레벨 디자인 (월드 배치) — v2: 중앙 허브 + 사방 해금

원칙 (2026-09-19 확정): ① 관리 영역은 플롯 **중앙**, ② 구역은 허브에서 **북·동·남·서로 점점 해금**, ③ **해금 전 구역은 존재하지 않음**(바닥·판매대·패드·작물 모두 미생성, 허브 가장자리의 게이트 🔒만 보임).

```
                 [Z1 북: 밀·벼]
                       │ 🔒
 [Z4 서: 티저] 🔒 ── (허브: 스폰·🎁·⚒️·🛒) ── 🔒 [Z2 동: 옥수수·감자]
                       │ 🔒
                 [Z3 남: 과수원]
```

- 플롯 200×200, 3×2 격자(간격 230), 플롯 둘레 흰 말뚝 울타리. 공용 광장 없음. 플롯 사이는 모래색 통로, 격자 바깥은 나무·바위 띠.
- 허브: 반지름 24 크림색 동심원 포장, 가로등 6, 스폰 (0,0,-6)에서 북쪽(Z1)을 바라봄, 🎁 (-14,0,4) · ⚒️ (14,0,4) · 🛒 (0,0,15).
- 구역 프레임: 허브가 -Z, 구역이 +Z로 뻗는 좌표에 배치를 적고 구역별 yaw(북 180°, 동 90°, 남 0°, 서 -90°)로 회전. 원점은 허브 중심에서 34, 게이트는 구역 프레임 (0,0,-14) = 허브 가장자리(중심에서 20).
- 구역 내부(프레임 좌표): Field1 (0,0,12), Field2 (0,0,44), 판매대 (-18,0,6), 스탯 패드 (-18,0,16/20/24), 확장 패드 (18,0,6), 일꾼 패드 (18,0,14), 2번째 작물 패드 = Field2 중심, 구역 바닥 60×70, 장식 3곳. Z2는 💧 패드 (-18,0,28)와 풍차.
- 해금 흐름: `PadService` 게이트 완료 → `ZoneUnlocked` 시그널 → `ZoneService`가 바닥·판매대·장식(`PlotService:BuildZone`) → 패드(`PadService:BuildZonePads`) → 작물(`CropService:BuildZone`) 순으로 생성하고 클라에 `ZoneUnlocked` 전송. 클라 `ZoneFX`는 새 구역 파트들이 땅에서 솟아오르는 연출 + 카메라 2초 팬.
- 접속 시 `ZoneService`가 데이터의 해금 구역만 생성. 플롯 반납 시 구역 오브젝트 전부 제거(게이트·허브는 유지).
...
| 레이어 | Y (윗면) | 대상 |
|---|---|---|
| World | 0.00 | 월드 바닥 (900×900) |
| Path | 0.06 | 플롯 사이 통로 |
| Plot | 0.10 | 플롯 바닥 (= `Layout.GroundY`, 오브젝트 기준면) |
| HubRing | 0.14 / 0.18 / 0.22 | 허브 동심원 3겹 |
| Zone | 0.16 | 구역 바닥 |
| Tile | 0.32 | 밭 타일 (작물이 이 위에 선다) |
| Ring | 0.40 | 수확 반경 링 |
| Dash | 0.52 | 가이드 화살표 대시 |

원인이었던 버그: 월드 바닥과 플롯 바닥의 윗면이 둘 다 0.1 로 완전히 겹쳐 있었다.
