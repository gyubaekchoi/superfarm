...
- 광장 지름 60 studs, 플롯 6개가 반경 180 studs 원형 배치. 플롯 크기 200×200 (플롯 로컬 중심 (40,0,50)).
- 구역 원점(플롯 로컬): Z1 (0,0,0) · Z2 (75,0,0) · Z3 (0,0,85) · Z4 (75,0,85). 구역 내부 배치는 `Layout.Common`을 공유하고 구역별로 덮어쓴다 (`Layout.pos(zone, key)`).
- 게이트(플롯 로컬): Z2 (38,0,0) x축 통과 · Z3 (0,0,58) z축 통과 · Z4 (75,0,58) 🚧 티저. 게이트 = 패드(5×0.4×5) + 기둥 2개.
- 미해금 구역은 반투명 안개 파트(78×24×90, ForceField)로 덮고, 해금 시 클라이언트가 1.8초 페이드 + 카메라 2초 팬 후 서버가 제거.
- **플롯 내부**(로컬 좌표, Z1 원점 기준)

| 오브젝트 | 위치 | 비고 |
|---|---|---|
| 스폰 | (−12, 0, 0), 밭을 향함 | 밭 가장자리까지 8 studs |
| Z1 밀밭 중심 | (0, 0, 0) | 5×7까지 확장 → 20×28 |
| Z1 벼밭 중심 | (0, 0, 32) | 잠김 시 잡초+🔒 패드 |
| Z1 판매대 | (−16, 0, 14) | 밭 가장자리 12 studs |
| 패드 3종(👟🌾🎒) | (−16, 0, −6/−10/−14) 일렬 | 스폰 옆 → 첫 방문 자연스러움 |
| 밭 확장 패드 | 밭 모서리 (12, 0, −16) | |
| 일꾼 패드 | (−16, 0, 20) | 판매대 옆 |
| Z2 게이트 | (38, 0, 0) 문 + 안개 | Z2 원점 (75,0,0) |
| Z3 게이트 | (0, 0, 58) | Z3 원점 (0,0,85) |
| Z4 게이트(티저) | (75, 0, 58) 🔒🚧 | |

- **광장 ⚒️ 강화대와 🎁 상자는 플롯 입구에서 20 studs 이내**. 강화 후 바로 밭으로 뛰어들 수 있어야 한다(§7-7 P4).
- 안개: 미해금 구역은 `Atmosphere` 대신 구역 위 반투명 파트 + 파티클(퍼포먼스). 해금 시 파트 투명도 1.0 트윈 2s.
